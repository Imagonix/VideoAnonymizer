...
- **Anonymization scope hierarchy** (materialized-track model, no track entity): effective blur = `OccurrenceBlurSizePercentOverride ?? BlurSizePercentOverride ?? Video.BlurSizePercent`; segment pre/post = `first/last PreBufferMsOverride/PostBufferMsOverride ?? TrackTimeBufferMsOverride ?? Video.TimeBufferMs`. `BlurSizePercentOverride` and `TrackTimeBufferMsOverride` are materialized on every occurrence in a track; occurrence-level blur lives only on one stored occurrence. Track-wide edits bulk-update every occurrence through the action queue; server/streamed objects arrive normalized.
