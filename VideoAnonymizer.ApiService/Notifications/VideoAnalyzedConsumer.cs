...
namespace VideoAnonymizer.ApiService.Notifications
{
    public class VideoAnalyzedConsumer : MessageConsumer<AnalyzedVideo>
    {
        ...
        {
            await _hub.Clients.All.SendAsync(SharedConstants.SignalR.Messages.VideoAnalyzed, new LongRunningJobFinishedMessage() { JobId = message.JobId, Status = "completed" }, cancellationToken);
        }
    }
}
